...
(function() {
    ...
    function submitOrder() {
        ...
        function processOrder(imageUrl) {
        var hasImage = imageUrl && imageUrl.length > 0;
        var orderRecord = { name: name, email: email, phone: phone, items: items, total: total, date: new Date().toLocaleString(), image: imageUrl || '' };
        try {
        ...
        fetch(API_BASE + '/api/orders', {
            ...
            body: JSON.stringify({ name: name, email: email, phone: phone, items: items, total: total, image: imageUrl || '' })
        }).then(function(r) {
        ...
        if (imageFile) {
            ...
            fetch(API_BASE + '/api/upload/order', {
                method: 'POST',
                body: fd
            }).then(function(r) {
                if (!r.ok) return processOrder('');
                return r.json();
            }).then(function(resp) {
                processOrder(resp.url || '');
            }).catch(function() {
                processOrder('');
            });
        } else {
